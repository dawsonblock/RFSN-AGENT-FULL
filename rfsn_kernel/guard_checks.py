"""Runtime guard checks on kernel state transitions.

Defines guard clauses that must hold before and after every kernel
decision step. These are runtime assertions on dictionary state,
not formal invariants in the academic sense.

Violations are logged and optionally fatal.
"""
...
class GuardViolation(Exception):
    """Raised when a kernel guard check fails."""

    def __init__(self, guard_name: str, message: str, context: dict):
        self.guard_name = guard_name
        self.context = context
        super().__init__(f"GUARD_VIOLATION [{guard_name}]: {message}")


# Backward compatibility alias
InvariantViolation = GuardViolation
...
def check_guards(
    ...
    """Run all guard checks against the given state.

    Args:
        state: Current kernel state dict.
        strict: If True, raise GuardViolation on first failure.

    Returns:
        List of violation dicts with keys: name, message
    """
    violations = []
    for name, check_fn in _GUARDS:
        msg = check_fn(state)
        if msg is not None:
            violations.append({"name": name, "message": msg})
            if strict:
                raise GuardViolation(name, msg, state)

    return violations


# Backward compatibility aliases
check_invariants = check_guards


def check_pre(state: Dict[str, Any]) -> List[Dict[str, Any]]:
    """Pre-step guard check."""
    return check_guards(state, strict=_is_strict())
...
def register_guard(name: str, check_fn):
    """Register a custom guard check function.

    check_fn(state) should return None if OK, or an error message string.
    """
    _GUARDS.append((name, check_fn))


# Backward compatibility alias
register_invariant = register_guard
